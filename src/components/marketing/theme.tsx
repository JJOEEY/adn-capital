/**
 * Shared marketing theme: fonts, scoped CSS, Nav, Footer, Shell. Used by the real
 * marketing routes (/, /gioi-thieu, /lien-he, /pricing, /products, /auth).
 ...
 * forest-green brand #2E4D3D, tan #D6CDBB + sage #7D8471 supporting.
 * Dark mode keys off the app's global `.dark` class (ThemeProvider), not a local toggle.
 */
    ...
    <header className="sticky top-0 z-50 border-b border-[var(--hairline)] bg-[color-mix(in_srgb,var(--canvas)_90%,transparent)] backdrop-blur-sm">
      <div className="mx-auto flex h-[66px] max-w-[1180px] items-center justify-between px-5 sm:px-8">
        ...
        <nav className="dp-mono hidden items-center gap-8 text-[12.5px] uppercase tracking-[0.12em] text-[var(--ink-muted)] md:flex">
          <a className="transition-colors hover:text-[var(--ink)]" href="/products">Công cụ</a>
          <a className="transition-colors hover:text-[var(--ink)]" href="/gioi-thieu">Giới thiệu</a>
          <a className="transition-colors hover:text-[var(--ink)]" href="/pricing">Bảng giá</a>
          <a className="transition-colors hover:text-[var(--ink)]" href="/lien-he">Liên hệ</a>
        </nav>
        <div className="flex items-center gap-2.5">
          <ThemeToggle />
          <a href="/auth" className="hidden text-[15px] font-medium text-[var(--ink-muted)] transition-colors hover:text-[var(--ink)] sm:block">Đăng nhập</a>
          <a href="/auth?mode=register" className="dp-btn dp-btn-solid">Mở tài khoản</a>
        </div>
      </div>
    </header>
  );
...
const FOOTER_COLS: { title: string; links: { label: string; href: string }[] }[] = [
  {
    title: "Sản phẩm",
    links: [
      { label: "AIDEN", href: "/products/adn-stock" },
      { label: "ADN Radar", href: "/products/adn-radar" },
      { label: "Chỉ báo ART", href: "/products/adn-art" },
      { label: "Cổ phiếu & RS", href: "/products/adn-stock" },
      { label: "Xếp hạng RANK", href: "/products/adn-rank" },
      { label: "Nhịp thị trường", href: "/products/adn-pulse" },
    ],
  },
  {
    title: "Công ty",
  ...
  },
  {
    title: "Bắt đầu",
    links: [
      { label: "Làm bài test", href: "/#quiz" },
      { label: "Đăng nhập", href: "/auth" },
      { label: "Mở tài khoản", href: "/auth?mode=register" },
    ],
  },
];
      ...
      <div className="mx-auto max-w-[1180px] px-5 py-14 sm:px-8">
        <div className="grid gap-10 md:grid-cols-[1.5fr_1fr_1fr_1fr]">
          <div className="max-w-[34ch]">
            ...
            <div className="mt-4"><AppDownloadTrigger /></div>
          </div>
          {FOOTER_COLS.map((col) => (
            <div key={col.title}>
...
.dp-panel{ background:var(--surface); border:1px solid var(--hairline); border-radius:16px; box-shadow:0 2px 8px rgba(var(--shadow),.06); }
...
/* ── Dark mode — Sylvan Logic accents on a NEUTRAL grey-black canvas (keyed off app .dark on <html>) ── */
.dark .dp-root{
  color-scheme:dark;
  --canvas:#0d0d0f;
  --cream:#151517;
...
.dark .dp-cta{ background:linear-gradient(150deg,#1c3527 0%,#284a37 55%,#356048 100%); border:1px solid rgba(143,201,160,0.20); box-shadow:0 28px 70px -34px rgba(0,0,0,0.8); }
.dark .dp-cover-base{ color:var(--ink); background:radial-gradient(64% 52% at 50% 40%, rgba(143,201,160,0.10), transparent 62%), linear-gradient(170deg,#202024,#161618); --stem:#9a9890; --leaf:#8fc9a0; --leaf-fill:rgba(143,201,160,0.15); --ground:rgba(236,231,219,0.16); --halo:rgba(143,201,160,0.22); --flower:#cfe3d5; --flower-center:#161618; }
.dark .dp-cover-vip{ background:radial-gradient(60% 50% at 50% 38%, rgba(150,210,170,0.22), transparent 62%), linear-gradient(165deg,#234a39,#16291d); }
.dark .dp-cover-vip, .dark .dp-cover-premium{ color:#ece7db; }
.dark .dp-btn-on-dark{ background:#ece7db; color:#163325; }
.dark .dp-quiz-num{ background:var(--mint); color:#8fc9a0; }
.dark .dp-dash{ border-color:rgba(255,255,255,0.08); }
/* foregrounds that assumed light-mode var values (cream=light, moss=dark) need pinning in dark */
.dark .text-\\[var\\(--cream\\)\\]{ color:#ece7db; }
.dark .text-\\[var\\(--moss\\)\\]{ color:#8fc9a0; }
`;
