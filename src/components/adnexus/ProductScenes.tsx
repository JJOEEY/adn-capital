...
export function ProductSceneVisual({ scene, compact = false }: Props) {
      ...
      <SceneFrame compact={compact} eyebrow="NexPulse" title="Bức tranh thị trường">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {["HOSE 18,420 tỷ", "HNX 1,120 tỷ", "UPCOM 840 tỷ"].map((item) => (
            <div key={item} className="rounded-2xl border bg-white p-4 text-sm font-black dark:bg-white/5" style={{ borderColor: "var(--border)", color: "var(--text-primary)" }}>{item}</div>
          ))}
        </div>
      </SceneFrame>
    );
          ...
          <div className="rounded-[1.75rem] border bg-white p-5 dark:bg-white/5" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3">
              {["Entry 56.0", "Mục tiêu 59.9", "Sai điểm 54.3"].map((item) => (
                <div key={item} className="rounded-2xl border p-4 text-center text-sm font-black" style={{ borderColor: "var(--border)", color: "var(--text-primary)" }}>{item}</div>
              ))}
            </div>
          </div>
        </div>
  ...
  if (scene === "lab" || scene === "flow" || scene === "sentinel") {
    ...
    return (
      <SceneFrame compact={compact} eyebrow={scene === "lab" ? "NexLab" : scene === "sentinel" ? "NexSentinel" : "NexFlow"} title="Theo dõi có bằng chứng">
      ...
      <SceneFrame compact={compact} eyebrow="AIDEN Advisory" title="Chat như một ứng dụng nhắn tin">
        <div className="rounded-[1.75rem] border bg-white p-5 dark:bg-white/5" style={{ borderColor: "var(--border)" }}>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="max-w-[82%] rounded-3xl bg-[var(--surface-2)] px-5 py-3 text-sm" style={{ color: "var(--text-primary)" }}>AIDEN sẽ tách bối cảnh kỹ thuật, cơ bản, dòng tiền và rủi ro. Dữ liệu nào thiếu sẽ được ghi rõ.</div>
