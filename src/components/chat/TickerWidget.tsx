...
function TEIGauge({ value }: { value: number }) {
  ...
  const strokeColor = value >= 4 ? "#f87171" : value >= 2.5 ? "#fbbf24" : "#34d399";
...
export function TickerWidget({ ticker, data }: TickerWidgetData) {
  ...
  const ta       = data.technical?.stats;
  const fa       = data.fundamental?.stats;
  const teiScore = data.behavior?.teiScore ?? 2.5;
  const isUp     = (ta?.price?.changePct ?? 0) >= 0;
  const signal   = data.signal;

  const teiColor = teiScore >= 4 ? "text-red-400" : teiScore >= 2.5 ? "text-yellow-400" : "text-emerald-400";
  ...
  return (
    ...
    >
      <Card glass glow={isUp ? "emerald" : "red"} className="overflow-hidden">
        ...
        {/* ── Signal Banner ──────────────────────────────────────── */}
        {signal && (
          <div className="px-5 py-2 bg-yellow-500/[0.06] border-b border-yellow-500/[0.15] flex items-center gap-2">
            <Zap className="w-3 h-3 text-yellow-400 flex-shrink-0" />
            <p className="text-[10px] text-yellow-300 font-bold">
              Quant Signal: <span className="font-black">{signal.type?.replace(/_/g, " ")}</span>
              {signal.entryPrice ? ` · Entry: ${signal.entryPrice.toLocaleString("vi-VN")}` : ""}
            </p>
          </div>
        )}
            ...
            >
              ...
              {activeTab === "ta" && (
                <>
                  ...
                  <AIInsightBlock insight={data.technical?.aiInsight ?? null} />
                </>
              )}
              ...
              {activeTab === "fa" && (
                <>
                  ...
                  <AIInsightBlock insight={data.fundamental?.aiInsight ?? null} />
                </>
              )}
                ...
                <>
                  {data.news?.length ? (
                    ...
                    <div className="py-8 text-center">
                      ...
                      <p className="text-xs text-slate-600">Chưa có tin tức từ FiinQuant</p>
                      <p className="text-[10px] text-slate-700 mt-1">Dùng <span className="font-mono text-slate-500">/news {ticker}</span> để xem toàn bộ</p>
                    </div>
                  )}
                </>
              )}
              ...
              {activeTab === "behavior" && (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="w-full space-y-1">
                    {[
                      { label: "Hưng phấn cực độ (≥ 4.0)", color: "text-red-400", icon: AlertTriangle },
                      { label: "Trung tính (1.0 – 4.0)", color: "text-yellow-400", icon: Minus },
