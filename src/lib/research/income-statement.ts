...
import { getVnstockDataBridgeUrl } from "@/lib/vnstockClient";
...
async function fetchPeriodsFromBridge(baseUrl: string, code: string): Promise<IncomeStatementPeriod[] | null> {
  try {
    const res = await fetch(`${baseUrl}/api/v1/income-statement/${code}`, {
      cache: "no-store",
