 ...
 * Step 3: AIDEN/NexPilot card (Gemini — CHỈ gọi khi signal FIRST-TIME NEW/RADAR)
 *
...
// ═══════════════════════════════════════════════════════════════════
//  Step 3: AIDEN/NexPilot message (Gemini)
...
async function callGeminiReasoning(params: {
  ...
  const prompt = `Bạn là AIDEN Analyst của ADNexus, vận hành bởi ADN Capital.
...
function buildChecklist(raw: RawScannerSignal, tier: SignalTier): string {
  const checks: string[] = [];
  if (tier === "LEADER") {
    checks.push(`${raw.rsRating && raw.rsRating >= 85 ? "✅" : "⬜"} NexRank ≥ 85 (${raw.rsRating ?? "N/A"})`);
...
export async function processSignals(
  ...
  for (const raw of rawSignals) {
    ...
    // Step 3: AIDEN/NexPilot message (Gemini) — CHỈ gọi cho signal MỚI
