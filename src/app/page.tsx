...
const safetyBullets = [
  "Tín hiệu gốc đến từ bộ quét và dữ liệu kiểm soát, không phải do AI tự bịa.",
  `Telegram, app, ${PRODUCT_NAMES.dashboard} và ${PRODUCT_NAMES.brokerWorkflow} đọc cùng một bản ghi để tránh lệch thông tin.`,
  "NexLink/DNSE chỉ ở chế độ pilot nội bộ, chưa public đặt lệnh thật.",
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <Link href="/auth" className="rounded-full border px-4 py-2 text-sm font-bold" style={{ borderColor: "var(--border)", color: "var(--text-primary)", background: "var(--surface)" }}>
            Đăng nhập
          </Link>
      ...
      {open ? (
        <div className="border-t px-4 pb-4 md:hidden" style={{ borderColor: "var(--border)", background: "var(--page-surface)" }}>
          <div className="grid gap-2 pt-3">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="rounded-2xl px-4 py-3 text-sm font-bold" style={{ background: "var(--surface)", color: "var(--text-primary)" }} onClick={() => setOpen(false)}>
                {link.label}
              </Link>
            ))}
            <Link href="/auth" className="rounded-2xl px-4 py-3 text-sm font-bold" style={{ background: "var(--primary)", color: "#EBE2CF" }} onClick={() => setOpen(false)}>
              Đăng nhập
            </Link>
          </div>
        </div>
...
function Hero() {
  return (
    <section className="relative overflow-hidden px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-28">
      <div className="absolute inset-0 -z-10" style={{ background: "radial-gradient(circle at 80% 15%, rgba(46,77,61,0.18), transparent 34%), radial-gradient(circle at 15% 20%, rgba(160,132,92,0.13), transparent 32%)" }} />
            ...
            <div className="rounded-[1.5rem] border p-5" style={{ background: "linear-gradient(145deg, var(--surface-2), var(--surface))", borderColor: "var(--border)" }}>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border p-4" style={{ borderColor: "var(--border)", background: "var(--primary-light)" }}>
                  <p className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: "var(--primary)" }}>Tỷ trọng tham khảo</p>
                  <p className="mt-3 text-4xl font-black">10%</p>
                  <p className="mt-2 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>Liên kết với quy mô danh mục khi xem trước hành động.</p>
                </div>
                <div className="rounded-2xl border p-4" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
                  <p className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: "var(--text-muted)" }}>Nguyên tắc</p>
                  <p className="mt-3 text-4xl font-black">SAFE</p>
                  <p className="mt-2 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>AI chỉ giải thích; dữ liệu và rủi ro đi qua rule xác định.</p>
                </div>
              </div>

              <p className="mt-5 text-xs leading-5" style={{ color: "var(--text-muted)" }}>
                Minh họa public. Không hiển thị tài khoản thật, không tự động đặt lệnh và không gọi DNSE runtime.
              </p>
            </div>
          </div>
        </motion.div>
      ...
      <section id="workflow" className="border-y px-4 py-14 sm:px-6 lg:px-8" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-xs font-black uppercase tracking-[0.22em]" style={{ color: "var(--primary)" }}>Quy trình ADNexus</p>
            <h2 className="mt-3 text-3xl font-black tracking-[-0.04em] sm:text-5xl">Từ dữ liệu đến hành động, không bỏ qua kỷ luật.</h2>
          </div>
      ...
      <section id="products" className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div className="max-w-3xl">
              <p className="text-xs font-black uppercase tracking-[0.22em]" style={{ color: "var(--primary)" }}>Bộ công cụ</p>
              <h2 className="mt-3 text-3xl font-black tracking-[-0.04em] sm:text-5xl">Một nền tảng, nhiều bề mặt sử dụng.</h2>
            </div>
              ...
              return (
                <Link key={card.name} href={card.href} className="group block">
                  <article className="h-full rounded-[1.5rem] border p-5 transition duration-200 group-hover:-translate-y-1" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
                    ...
                    <p className="mt-5 text-xs font-black uppercase tracking-[0.18em]" style={{ color: "var(--text-muted)" }}>{card.label}</p>
                    <h3 className="mt-2 text-2xl font-black">{card.name}</h3>
                    <p className="mt-3 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>{card.body}</p>
                  </article>
                </Link>
              );
            })}
      ...
      <section id="safety" className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[2rem] border p-6" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
            <p className="text-xs font-black uppercase tracking-[0.22em]" style={{ color: "var(--primary)" }}>Nguyên tắc vận hành</p>
          ...
          <div className="grid gap-3">
            {safetyBullets.map((bullet) => (
              <div key={bullet} className="flex gap-3 rounded-2xl border p-4" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
                ...
                <p className="text-sm leading-6" style={{ color: "var(--text-secondary)" }}>{bullet}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[2rem] border p-6 sm:p-8" style={{ background: "linear-gradient(135deg, var(--primary), #173627)", borderColor: "rgba(255,255,255,0.12)", color: "#EBE2CF" }}>
          <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em]" style={{ color: "rgba(235,226,207,0.72)" }}>Bắt đầu</p>
