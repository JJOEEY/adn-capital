...
export async function POST(req: NextRequest) {
  ...
  const baseFallbackResponse = createFallbackRunError({
    ...
    error: "Bridge scanner run endpoint unavailable",
    warnings: [
      "Deterministic scanner source is unavailable.",
      "AI insight generation is blocked until deterministic scan results are available.",
    ],
    source: "fallback",
  });
  ...
  try {
    const res = await fetch(`${BRIDGE}/api/v1/providers/scanner/run`, {
      method: "POST",
    ...
    return NextResponse.json(
      createFallbackRunError({
        providerKey: parsedRequest.providerKey,
        error: payload?.error || payload?.message || `Bridge scanner run failed with HTTP ${res.status}`,
        source: "bridge",
      }) satisfies ProviderRunResponse,
      { status: 502 },
    );
    ...
    console.warn(
      `[providers.scanner.run] bridge run unavailable for provider=${parsedRequest.providerKey}`,
      error,
    );
  }
